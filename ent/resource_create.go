...
func (rc *ResourceCreate) SetMessage(m *map[string]interface{}) *ResourceCreate {
	rc.mutation.SetMessage(m)
	return rc
}

// AddElementIDs adds the "elements" edge to the Element entity by IDs.
func (rc *ResourceCreate) AddElementIDs(ids ...int) *ResourceCreate {
	rc.mutation.AddElementIDs(ids...)
	return rc
}

// AddElements adds the "elements" edges to the Element entity.
func (rc *ResourceCreate) AddElements(e ...*Element) *ResourceCreate {
	ids := make([]int, len(e))
	for i := range e {
		ids[i] = e[i].ID
	}
	return rc.AddElementIDs(ids...)
}
...
func (rc *ResourceCreate) check() error {
	if _, ok := rc.mutation.Message(); !ok {
		return &ValidationError{Name: "message", err: errors.New(`ent: missing required field "Resource.message"`)}
	}
	return nil
}
...
func (rc *ResourceCreate) createSpec() (*Resource, *sqlgraph.CreateSpec) {
	...
	if nodes := rc.mutation.ElementsIDs(); len(nodes) > 0 {
