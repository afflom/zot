...
type ResourceQuery struct {
	...
	withNamedElements map[string]*ElementQuery
...
func (rq *ResourceQuery) QueryElements() *ElementQuery {
	query := (&ElementClient{config: rq.config}).Query()
...
func (rq *ResourceQuery) Clone() *ResourceQuery {
	...
	return &ResourceQuery{
		...
		withElements: rq.withElements.Clone(),
...
func (rq *ResourceQuery) WithElements(opts ...func(*ElementQuery)) *ResourceQuery {
	query := (&ElementClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withElements = query
	return rq
}
...
func (rq *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	...
	if query := rq.withElements; query != nil {
		if err := rq.loadElements(ctx, query, nodes,
			func(n *Resource) { n.Edges.Elements = []*Element{} },
			func(n *Resource, e *Element) { n.Edges.Elements = append(n.Edges.Elements, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedElements {
		if err := rq.loadElements(ctx, query, nodes,
			func(n *Resource) { n.appendNamedElements(name) },
			func(n *Resource, e *Element) { n.appendNamedElements(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *ResourceQuery) loadElements(ctx context.Context, query *ElementQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Element)) error {
	...
	query.Where(predicate.Element(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(resource.ElementsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.resource_elements
		if fk == nil {
			return fmt.Errorf(`foreign-key "resource_elements" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "resource_elements" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedElements tells the query-builder to eager-load the nodes that are connected to the "elements"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *ResourceQuery) WithNamedElements(name string, opts ...func(*ElementQuery)) *ResourceQuery {
	query := (&ElementClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedElements == nil {
		rq.withNamedElements = make(map[string]*ElementQuery)
	}
	rq.withNamedElements[name] = query
	return rq
}
